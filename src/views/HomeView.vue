<template>
  ...
  <section class="todo-list-section">
    ...
    <TodoAddBtn @todo-added="addToDo" class="home-addtodo-btn"> </TodoAddBtn>
  </section>
</template>
...
export default {
  ...
  computed: {
    ...
    displayProgressSummary() {
      const numCompletedItems = this.ToDoItems.filter(
        (item) => item.done
      ).length;
    ...
    displayProgressPercent() {
      ...
      const percentProgress = Math.floor(
        (`${numCompletedItems}` / `${this.ToDoItems.length}`) * 100
      );
